...
const canKeepAlive = computed((): string[] => {
  // 获取 app1 开头的缓存信息
  const appCacheViews = cacheViews.value.filter((item) => item.path.startsWith('/app1/'));
  // 将 app1 的缓存信息进行映射，获取到实际 router
  const cacheViewNames = appCacheViews.map((item) => routes.find((route) => `/app1${route.path}` === item.path)) || [];
  console.log(cacheViewNames.map((item) => (item?.name ?? '') as string));
