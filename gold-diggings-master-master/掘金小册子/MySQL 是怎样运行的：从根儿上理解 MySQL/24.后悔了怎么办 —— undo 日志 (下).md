...
![image_1d79gnaudfg2psk2l7mi1146r20.png-104.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fcc0ca928?w=1115&h=463&f=png&s=107286)
...
![image_1d79o8ra61jh6k0k6af13vpg329.png-103.9kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fcc9ccd4f?w=978&h=515&f=png&s=106418)
...
![image_1d79nq0ge1gn7pmcaa7gma1uh62q.png-71kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fcc3c65f1?w=1067&h=377&f=png&s=72751)
...
![image_1d79ec33apm47brq901sur3bi16.png-63.2kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fd0d7f9fa?w=938&h=589&f=png&s=64680)
...
![image_1d79ohe8u1uqgh1e978nji1ip2m.png-71.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fd2ad0290?w=560&h=431&f=png&s=73515)
...
*   `TRX_UNDO_PAGE_TYPE`：本页面准备存储什么种类的`undo日志`。
    ...
    *   `TRX_UNDO_UPDATE`（使用十进制`2`表示），除了类型为`TRX_UNDO_INSERT_REC`的`undo日志`，其他类型的`undo日志`都属于这个大类，比如我们前边说的`TRX_UNDO_DEL_MARK_REC`、`TRX_UNDO_UPD_EXIST_REC`啥的，一般由`DELETE`、`UPDATE`语句产生的`undo日志`属于这个大类。
    ...
    > 小贴士： 之所以把undo日志分成两个大类，是因为类型为TRX\_UNDO\_INSERT\_REC的undo日志在事务提交后可以直接删除掉，而其他类型的undo日志还需要为所谓的MVCC服务，不能直接删除掉，对它们的处理需要区别对待。当然，如果你看这段话迷迷糊糊的话，那就不需要再看一遍了，现在只需要知道undo日志分为2个大类就好了，更详细的东西我们后边会仔细唠叨的。
...
*   `TRX_UNDO_PAGE_FREE`：与上边的`TRX_UNDO_PAGE_START`对应，表示当前页面中存储的最后一条`undo`日志结束时的偏移量，或者说从这个位置开始，可以继续写入新的`undo日志`。
    ...
    ![image_1d79s1ib21rku5fkq3to1e14313.png-54kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a7fd2fbc0f4?w=810&h=260&f=png&s=55289)
...
![image_1d79v7bib12041n9d1gpe1t8a10jc1g.png-60.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a8074556f5d?w=1172&h=381&f=png&s=62226)
...
![image_1d7a6t4va1e0c1l9t1hq51vlt183l2d.png-54.6kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a808197ad5b?w=1173&h=404&f=png&s=55954)

另外，设计`InnoDB`的大叔规定对普通表和临时表的记录改动时产生的`undo日志`要分别记录（我们稍后阐释为啥这么做），所以在一个事务中最多有4个以`Undo页面`为节点组成的链表：

![image_1d7bg5o7c3t11nch988lj51hsl9.png-106.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80840261ec?w=1107&h=633&f=png&s=109318)
...
![image_1d7blo9dp1m4tj1f1ke9te11654m.png-96.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80bf9229a6?w=1145&h=573&f=png&s=99142)
...
![image_1d7bp5ndt171bb6rkohot41e023.png-75.7kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80c187c8c2?w=694&h=361&f=png&s=77504)
...
![image_1d7brcccqah1rdn10573vh1onip.png-70.5kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80e0dadcce?w=915&h=555&f=png&s=72243)

可以看到这个`Undo`链表的第一个页面比普通页面多了个`Undo Log Segment Header`，我们来看一下它的结构：

![image_1d7bsbk8o15ja2bk1s9p11c51vs2p.png-80.2kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80e753a16e?w=642&h=555&f=png&s=82117)
...
![image_1d7cbktqb16oqih12mqmghn2a1m.png-82kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80ee6fb23f?w=964&h=605&f=png&s=83957)

这个`Undo Log Header`具体的结构如下：

![image_1d7cfr3cjsec64714qgucc1s8s9.png-122.8kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80f1d5b422?w=500&h=633&f=png&s=125765)
...
*   `TRX_UNDO_PREV_LOG`：上一组的`undo日志`在页面中开始的偏移量。
    ...
    > 小贴士： 一般来说一个Undo页面链表只存储一个事务执行过程中产生的一组undo日志，但是在某些情况下，可能会在一个事务提交之后，之后开启的事务重复利用这个Undo页面链表，这样就会导致一个Undo页面中可能存放多组Undo日志，TRX\_UNDO\_NEXT\_LOG和TRX\_UNDO\_PREV\_LOG就是用来标记下一组和上一组undo日志在页面中的偏移量的。关于什么时候重用Undo页面链表，怎么重用这个链表我们稍后会详细说明的，现在先理解TRX\_UNDO\_NEXT\_LOG和TRX\_UNDO\_PREV\_LOG这两个属性的意思就好了。
    
*   `TRX_UNDO_HISTORY_NODE`：一个12字节的`List Node`结构，代表一个称之为`History`链表的节点。
    
    > 小贴士： 关于History链表我们后边会格外详细的唠叨，现在先不用管哈。
...
![image_1d7cocjrk1dvm1ehg16da1di4mfb16.png-87kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80f3cc070f?w=1134&h=502&f=png&s=89063)
...
*   insert undo链表
    ...
    ![image_1d7el7sg0pje14rjc06rp51r8r1m.png-91.7kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a80f3f150f4?w=857&h=514&f=png&s=93950)
    ...
    > 小贴士： 当然，在重用Undo页面链表写入新的一组undo日志时，不仅会写入新的Undo Log Header，还会适当调整Undo Page Header、Undo Log Segment Header、Undo Log Header中的一些属性，比如TRX\_UNDO\_PAGE\_START、TRX\_UNDO\_PAGE\_FREE等等等等，这些我们就不具体唠叨了。
...
*   update undo链表
    ...
    ![image_1d7elkmtjdbv1kth1c9jk17is123.png-126.3kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a81007cdf9f?w=1003&h=560&f=png&s=129368)
...
![image_1d7gs2k8u1i9i15r71dg97fj10jv9.png-88kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a810434772a?w=676&h=541&f=png&s=90109)
...
*   `TRX_RSEG_MAX_SIZE`：本`Rollback Segment`中管理的所有`Undo页面`链表中的`Undo页面`数量之和的最大值。换句话说，本`Rollback Segment`中所有`Undo页面`链表中的`Undo页面`数量之和不能超过`TRX_RSEG_MAX_SIZE`代表的值。
    ...
    > 小贴士： 无限大其实也只是个夸张的说法，4个字节能表示最大的数也就是0xFFFFFFFF，但是我们之后会看到，0xFFFFFFFF这个数有特殊用途，所以实际上TRX\_RSEG\_MAX\_SIZE的值为0xFFFFFFFE。
...
![image_1d7214jha1cua1dgu1r6718091gbfm.png-27.4kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a810c4f1311?w=665&h=465&f=png&s=28093)

每个8字节的格子的构造就像这样：

![image_1d721oblche9a4pmng15p1dk313.png-9.9kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a810fe2ad15?w=362&h=161&f=png&s=10187)
...
![image_1d7h72gvlin31far16h11elj16tu1m.png-97kB](https://user-gold-cdn.xitu.io/2019/4/16/16a24a8116df4474?w=1070&h=618&f=png&s=99368)
