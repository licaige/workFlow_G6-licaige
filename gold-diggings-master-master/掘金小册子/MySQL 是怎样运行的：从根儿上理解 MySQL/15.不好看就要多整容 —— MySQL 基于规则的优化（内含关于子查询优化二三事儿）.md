...
*   从表`s1`的角度来看待，整个查询的意思其实是：对于`s1`表中的每条记录来说，如果该记录的`key1`列的值在子查询对应的物化表中，则该记录会被加入最终的结果集。画个图表示一下就是这样：
    ...
    ![image_1cvfj9up26i518t91li5ooq1r0u2d.png-84.9kB](https://user-gold-cdn.xitu.io/2019/5/6/16a8dda5369e68c5?w=741&h=566&f=png&s=86914)
    
*   从子查询物化表的角度来看待，整个查询的意思其实是：对于子查询物化表的每个值来说，如果能在`s1`表中找到对应的`key1`列的值与该值相等的记录，那么就把这些记录加入到最终的结果集。画个图表示一下就是这样：
    
    ![image_1cvfjg3os1oh1e3o5c11dhd1odd2q.png-67.4kB](https://user-gold-cdn.xitu.io/2019/5/6/16a8dda5394d3903?w=730&h=509&f=png&s=68994)
...
*   LooseScan execution strategy （松散索引扫描）
    ...
    ![image_1cvg8f3nst9n1amc14iljc3i4c37.png-110.3kB](https://user-gold-cdn.xitu.io/2019/5/6/16a8dda53aa776bf?w=959&h=440&f=png&s=112907)
